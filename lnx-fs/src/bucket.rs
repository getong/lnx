...
impl BucketOptions {
    async fn save_in_metastore(&self, metastore: &Metastore) -> Result<(), FileSystemError> {
        metastore.set_config_value("name", &self.name).await?;
        metastore.set_config_value("max_open_readers", &self.max_open_readers).await?;
        metastore.set_config_value("open_readers_time_to_idle", &self.readers_time_to_idle.as_secs()).await?;
        Ok(())
    }
    ...
    async fn load_from_metastore(
        ...
        let max_open_readers: usize = metastore
            ...
            .ok_or_else(|| {
                FileSystemError::Corrupted(
                    "Bucket required config key \"max_open_readers\" does not exist".to_string()
                )
            })?;
...
impl Bucket {
    ...
    /// is returned.
    pub async fn create(options: BucketOptions, runtime: RuntimeDispatcher) -> Result<Self, FileSystemError> {
    ...
    /// error is returned.
    pub async fn open(base_path: PathBuf, runtime: RuntimeDispatcher) -> Result<Self, FileSystemError> {
    ...
    async fn open_bucket_inner(
        ...
        let writer_options = load_writer_options(&metastore, paths.tablets_path.clone()).await?;        
...
async fn load_writer_options(metastore: &Metastore, base_path: PathBuf) -> Result<TabletWriterOptions, MetastoreError> {
    let max_tablet_size = metastore.get_config_value::<u64>("max_tablet_size").await?;
    let max_active_writers = metastore.get_config_value::<usize>("max_active_writers").await?;
